...
The runnable local app intentionally uses a no-credential in-memory runner so the full review flow works immediately. It calls an external HTTP endpoint and uses a visibly disclosed 450ms LLM stub, which is permitted by the brief when an API key is unavailable. The repository also contains the real Hasura GraphQL documents, migrations, metadata, Actions, cron trigger, and event trigger. In an Nhost deployment, point the Action handler at persistent Hasura mutations and use `STEP_RUN_PROGRESS` instead of the local polling adapter; the data model and authorization checkpoints are identical.
