...
The browser runner is intentionally local for immediate, reliable pause/resume interaction on serverless hosting; it does not background-poll or depend on process memory shared between functions. Its LLM node makes one request to a same-origin server route, which calls the OpenAI Responses API only when `OPENAI_API_KEY` is present. The key never reaches the browser; an explicitly marked deterministic fallback preserves the walkthrough when a secret or provider is unavailable. The repository also contains the real Hasura GraphQL documents, migrations, metadata, Actions, cron trigger, event trigger, and a server-side HTTP runner with retry. In an Nhost deployment, persist run state via Hasura mutations and use `STEP_RUN_PROGRESS`; the data model and authorization checkpoints are identical.
